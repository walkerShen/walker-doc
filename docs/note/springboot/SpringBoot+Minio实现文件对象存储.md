...
`mkdir minio`
#### 2、创建data config文件夹
进入minio文件夹中`cd minio/`
创建data和config文件夹
`mkdir data`
`mkdir config`
#### 3、在minio文件夹中创建docker-compose.yaml文件
```yaml
version: "3"
...
如果是up则代表ok了，如果不是的话，则在minio文件夹内，执行`docker-compose logs -f`查看日志
#### 6、访问连接
访问 ip:9001 即可
...
访问成功后的页面
之后使用docker-compose.yaml的配置文件的密码登录
...
```
### 2、添加配置
可以在yaml中配置参数，当然，如果有多环境的话，需要进行多环境配置
...
端口用错了，应该使用的是9000的，而不是9001,9001是界面的端口，9000才是api的端口
